...
import { queryReceivedInvoicesList } from './queries';
...
> {
  return action(async () => {
    const { companyId } = await requireCompanyAccess();
    // Shared with the SSR seed in received-invoices/page.tsx — see queries.ts.
    return queryReceivedInvoicesList(companyId, filters);
  });
}
