...
- **Slices 4–6 + Slice 0 — not started** (period lock + real кл.50/60 close, дневник/декларация, НАП export, onboarding/opening balances). With sales + purchases now posting, **Slice 4** (month close → 4532−4531 → кл.50/60) is the next high‑value step: it turns the VAT‑page Прогноза into the real number the owner asked for.
