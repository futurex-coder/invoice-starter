...
- **Slice 2 — Sales posting spine — ✅ DONE (sales only, C‑MVP‑1 single tax line).** Shipped: `BG_CHART_OF_ACCOUNTS` (66 accounts) + `VAT_OPERATIONS` registry (+ grounds‑derived keying); contра engine (`contra.ts`, reproduces the Microinvest example) with Microinvest display naming (411/1·2, 453/1·2, 70x); `journal_entries`/`journal_lines`/`journal_tax_lines`/`journal_sequences` (migration `0008`) with **balance + immutability triggers** (DB‑verified); `postInvoiceContra` (derive→persist one balanced immutable entry→lock source `accounted`); **posting‑existence guards** on cancel/delete/un‑account (keyed on posting existence, not `accountingStatus` — stress #1/#3/#4); **`reverseInvoiceContra`** (червено сторно counter‑entry, unlocks + re‑postable) with migration `0009` making the source‑unique index partial on `status <> 'reversed'`; **Меню Контиране** panel on the invoice detail page (gated Осчетоводи / Сторнирай). Tests: `contra.test.ts` (12), `posting.integration.test.ts` (2 — post + full reverse/re‑post lifecycle), `ContiranePanel.test.tsx` (5). Full suite 294.
  ...
  - **Deferred to later Slice‑2 polish:** the remaining Microinvest names/analytics for the full chart, Хронологичен + Оборотна read‑outs. (Basis picker landed with Slice 3.)
- **Slice 3 — Purchases posting (assisted‑manual) — ✅ DONE.** `getReceivedInvoiceContraPreview(id,{basis,noCredit})` / `postReceivedInvoiceContra` / `reverseReceivedInvoiceContra`: derive the balanced purchase statement (Dr 60x/30x/204/304 + Dr 453/1 / Cr 401 full credit; чл.70 no‑credit capitalises the VAT into the cost, кл.30, 0 deductible), persist one immutable entry (`sourceReceivedInvoiceId`), lock the source. Owner classifies via the purchase Меню Контиране panel (**Основание picker** + **чл.70 toggle**) on the received‑invoice detail page; the posted panel mirrors the booked classification. Zero‑VAT purchases are flagged + blocked (auto‑classification needs an accountant‑signed register‑null op). Received‑invoice guards (edit/un‑account/delete refuse a live контировка — stress #4 closed). Shared `writeReversalEntry` + `components/kontirovka/contra-ledger.tsx`. Tests: `purchase-posting.integration.test.ts` (5 — full‑credit lifecycle + guards + reverse/re‑post, чл.70, basis→304, posted‑truth, **edit lock**), `PurchaseContiranePanel.test.tsx` (4). Full suite 302 (309 after the edit‑lock guard).
  - **Deferred:** zero‑VAT purchase posting; supplier credit notes (negative); the „later period“ right‑to‑credit window (uses issue month, like sales) — unlocks with Slice 4.
- **Slice 4a — Posted‑VAT read‑out — ✅ DONE.** `getPostedVatForPeriod` (`lib/db/queries/vat-posted.ts`) sums `journal_tax_lines` over live document postings (`status=posted AND kind=document`) → изходящ − входящ = нето (кл.20/40/50‑60 proxy); the `kind=document` filter nets a сторно to zero (reversed original + `kind=reversal` counter‑entry both excluded). Surfaced as the „Осчетоводен ДДС (реален)“ strip in the VAT‑page month drill‑down (`MonthDnevnik`), next to the accrual Прогноза. Test: `posted-vat.integration.test.ts` (sale+purchase → нето; reverse nets out). Full suite 303. Also de‑flaked the kontirovka integration suite (per‑file company‑EIK leading digit: posting=9, purchase=8, posted‑vat=4).
- **Slice 4b (period lock + formal month‑close entry) — not started.** `accounting_periods` + `prevent_closed_period_posting`; a `kind='vat_close'` entry (Dr 4532 / Cr 4531 → 4539/4538); flip the VAT‑page Прогноза→real on close; then the `vatPeriod` override (deferred ДК / storno timing) can safely arrive. 4a already exposes the number read‑only; 4b makes it a filed, locked figure.
- **Slices 5–6 + Slice 0 — not started** (дневник/декларация as GROUP BY over `journal_tax_lines`, НАП export — both **hard‑gated on accountant клетка sign‑off §8.2**; onboarding/opening balances).
