...
- [dds-dnevnik-spec.md](dds-dnevnik-spec.md) — build-ready spec for the ДДС дневник продажби/покупки (НАП VAT ledgers): doc-type codes, data sources (invoices.totals JSONB + received_invoices flat cols), the money.ts reuse rules + reconciliation-to-getVatSummary invariant, and the compliance-precise клетка-mapping to verify before export. Supports the §2 headline feature. (2026-07-09)
